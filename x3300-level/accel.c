...
static void acc1_read(int *x, int *y, int *z) {
    signed char buf[3];
    i2c_read_bytes(0, ACC1, ACC1_X_DATA, (byte *) buf, 3);
...
static void acc2_read(int *x, int *y, int *z) {
    signed char buf[6];
    i2c_read_bytes(0, ACC2, ACC2_OUT|0x80,(byte *) buf, 6);
...
void accel_start(void) {
    ...
    if (i2c_try_read(0, ACC1, 0x0d, &buf) == OK) {
        i2c_write_reg(0, ACC1, ACC1_CTRL_REG1, 0x23); // 50Hz, 8 bit, Active
        accel = 1;
    } else if (i2c_try_read(0, ACC2, 0x0f, &buf) == OK) {
        i2c_write_reg(0, ACC2, ACC2_CTRL_REG1, 0x4f); // 50Hz, 8 bit, Active
        accel = 2;
    } else {
